...
func TestLinksFilteredByTransport(t *testing.T) {
	...
	defer transports[0].Drop()

	// Filter by the matching transport — should return 1 link.
	links, err := s1.Links(&zenoh.InfoLinksOptions{Transport: &transports[0]})
	assert.NoError(t, err)
	assert.Equal(t, 1, len(links))
	for _, l := range links {
		l.Drop()
	...
	defer s2Transports[0].Drop()

	linksFiltered, err := s1.Links(&zenoh.InfoLinksOptions{Transport: &s2Transports[0]})
	assert.NoError(t, err)
...
func TestTransportAccessors(t *testing.T) {
	...
	assert.Equal(t, clone.ZId().String(), transports[0].ZId().String())
	clone.Drop()

	// Verify original is still valid after clone drop
...
func TestLinkAccessors(t *testing.T) {
	...
	// Test Clone
	clone := links[0].Clone()
	assert.Equal(t, clone.Src(), links[0].Src())
	assert.Equal(t, clone.ZId().String(), links[0].ZId().String())
	clone.Drop()

	// Verify original is still valid after clone drop
	assert.Equal(t, s2.ZId().String(), links[0].ZId().String())
	assert.Equal(t, links[0].Src(), links[0].Src()) // Verify Src still accessible
}
