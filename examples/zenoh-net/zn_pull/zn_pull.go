...
func main() {
	...
	var args struct {
		Selector string `default:"/zenoh/examples/**" arg:"-s" help:"The selector to be used for issuing the pull subscription"`
	...
	sub, err := s.DeclareSubscriber(args.Selector, znet.NewSubMode(znet.ZNPullMode), listener)
	if err != nil {
		panic(err.Error())
	}
