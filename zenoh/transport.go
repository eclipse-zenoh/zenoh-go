...
type Transport struct {
	transport *C.z_owned_transport_t
}

// Return the Zenoh ID of the remote peer.
func (t *Transport) ZId() Id {
	return Id{id: C.z_transport_zid(C.z_transport_loan(t.transport))}
}

// Return what kind of Zenoh entity is at the other end of the transport.
func (t *Transport) WhatAmI() WhatAmI {
	return WhatAmI(C.z_transport_whatami(C.z_transport_loan(t.transport)))
}

// Return whether Quality of Service is enabled for this transport.
func (t *Transport) IsQos() bool {
	return bool(C.z_transport_is_qos(C.z_transport_loan(t.transport)))
}

// Return whether this transport is multicast.
func (t *Transport) IsMulticast() bool {
	return bool(C.z_transport_is_multicast(C.z_transport_loan(t.transport)))
}

// Return whether shared memory is enabled for this transport.
func (t *Transport) IsShm() bool {
	return bool(C.z_transport_is_shm(C.z_transport_loan(t.transport)))
}

// Clone the transport.
func (t *Transport) Clone() Transport {
	var dst C.z_owned_transport_t
	C.z_transport_clone(&dst, C.z_transport_loan(t.transport))
	return Transport{transport: &dst}
}

// Drop and invalidate the transport.
func (t *Transport) Drop() {
	C.z_transport_drop(C.z_transport_move(t.transport))
}
...
func zenohTransportEventsCallback(event *C.z_loaned_transport_event_t, context unsafe.Pointer) {
	...
	evt := TransportEvent{
		...
		isShm:       bool(C.z_transport_is_shm(loanedTransport)),
	}
...
func zenohTransportCallback(transport *C.z_loaned_transport_t, context unsafe.Pointer) {
	...
	C.z_transport_clone(&owned, transport)
	(*internal.ClosureContext[Transport])(context).Call(Transport{transport: &owned})
}
